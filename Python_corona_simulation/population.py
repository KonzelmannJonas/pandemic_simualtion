...
def save_data(population, pop_tracker, parameters):
    ...
    #np.savetxt('infected.txt', pop_tracker.infectious)

    with open('data/%i/parameters.txt'%num_files,'w') as parameter_file:
        parameter_file.write('Population size: %i\n'%parameters[0])
        parameter_file.write('infection_range: %f\n'%parameters[1])
        parameter_file.write('infection_chance: %f\n'%parameters[2])
        parameter_file.write('recovery_duration low: %i\n'%parameters[3][0])
        parameter_file.write('recover_duration high %i\n'%parameters[3][1])
        parameter_file.write('mortality_chance: %f\n'%parameters[4])
