...
dead: %i, of total: %i' % (self.frame, self.pop_tracker.susceptible[-1], self.pop_tracker.infectious[-1],
        ...
        #sys.stdout.write('\r')
        #sys.stdout.write("Frame: %i,   Simulation Step: %i" % (self.frame, self.Config.simulation_steps))
    ...
    def run(self):
        ...
        if self.Config.save_data:
            parameters_list = [self.Config.pop_size, self.Config.infection_range, self.Config.infection_chance,self.Config.recovery_duration, self.Config.mortality_chance]
            save_data(self.population, self.pop_tracker, parameters_list)
            #np.savetxt('infected.txt', self.pop_tracker.infectious)
